...
import torch.nn.functional as F#工夫4損失関数をKLdivergenceに
...
def train(model, dataloader, optimizer, criterion, device):
    ...
    for image, question, answers, mode_answer in dataloader:
        ...
        #損失関数をKLダイバージェンスに変えることでここも変わる
        #log_softmax関数を適用
        log_softmax_pred = F.log_softmax(pred, dim=1)

        #one-hotエンコードされたターゲットに対してsoftmaxを適用
        softmax_target = F.softmax(F.one_hot(mode_answer, num_classes=pred.size(1)).float(), dim=1)

        loss = criterion(log_softmax_pred, softmax_target)
